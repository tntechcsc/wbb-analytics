...
useEffect(() => {
  const fetchInitialData = async () => {
    try {
      ...
      try {
        const tempoResponse = await fetch('http://localhost:3001/api/tempoEvents/byPlayer/' + playerData[0]._id);
    ...
    try{
      ...
      const sessionData = await sessionResponse.json(); //This comes out as an object with an array of practices as an attribute
      const formattedSessions = sessionData.practices.map(session => {
    ...
    /*try {
      ...
      console.error("Failed to fetch tempo data:", error);
    }*/
    /*try {
      const drillResponse2 = await fetch('http://localhost:3001/api/drills');
      const drillData2 = await drillResponse2.json();
      console.log("These are drills:")
      console.log(drillData2);
    } catch (error) {
      console.error("Failed to fetch drill data:", error);
    }*/
    try {
      const shotsResponse = await fetch('http://localhost:3001/api/shotEvents/');
      const shotsData = await shotsResponse.json(); //This comes out of the route as an object with an array containing all the shots. Odd.
      //console.log("These are shots:")
      //console.log(shotsData);
      setAllShots(shotsData.shots);
    } catch (error) {
      console.error("Failed to fetch tempo data:", error);
    }
  };
