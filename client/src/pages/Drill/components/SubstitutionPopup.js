...
function SubstitutionPopup({ isOpen, onClose, onSubstitute, playersOnCourt, allPlayers }) {
    const ignoreClickAwayRef = React.useRef(false);

    const handleClose = () => {
        if (ignoreClickAwayRef.current) {
            ignoreClickAwayRef.current = false; // Reset and ignore this close attempt
            return;
        }
        onClose(); // Actual close logic
    };

    React.useEffect(() => {
        if (isOpen) {
            ignoreClickAwayRef.current = true; // Popup just opened, ignore the next click away
            setTimeout(() => {
                ignoreClickAwayRef.current = false; // Reset after a short delay
